...
<div id="post"><!--网页中间部分文章信息开始-->
	...
	<div id="cmsl"><!--循环输出左侧的分类模块-->
		<?php $display_categories = explode(',', get_option('vfhky_catl') ); foreach ($display_categories as $category) { ?>
		<?php
			query_posts( array(
				'showposts' => 1,
				'cat' => $category,
		...
		<div class="entry_box_h">
			...
			<div class="cat_post_box">
		  		<?php
					query_posts( array(
						'showposts' => get_option('vfhky_cat_n'),
						'cat' => $category,
						'offset' => 1,
						'post__not_in' => $do_not_duplicate
						)
				...
				<ul><li>
					...
					<a href="<?php the_permalink() ?>" rel="bookmark"  title="<?php $post_left_title = $post->post_title;echo $post_left_title; ?>"><?php echo mb_strimwidth($post_left_title,0,30); ?></a>
				</li></ul>
				<?php endwhile; ?>
			</div>
				<div class="ption">
	...
	<div id="cmsr"><!--循环输出右侧的分类模块-->
		<?php $display_categories = explode(',', get_option('vfhky_catr') ); foreach ($display_categories as $category) { ?>
		<?php
			query_posts( array(
				'showposts' => 1,
				'cat' => $category,
		...
		<div class="entry_box_h">
			...
			<div class="cat_post_box">
				...
				<ul><li>
					<span class="hoem_date"><?php the_time('Y-m-d'); ?></span>
					<a href="<?php the_permalink() ?>" rel="bookmark"  title="<?php $post_right_title = $post->post_title;echo $post_right_title; ?>"><?php echo mb_strimwidth($post_right_title,0,30); ?></a>
				</li></ul>
				<?php endwhile; ?>
			</div>
				<div class="ption">
