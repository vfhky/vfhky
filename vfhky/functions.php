<?php
if (get_option('vfhky_cut_img') == '关闭') { echo ''; } 
...
function article_index($content) {
    ...
    if (preg_match_all($r, $content, $matches)) {
        foreach ($matches[2] as $num => $title) {
            ...
            if ($matches[1][$num] == 'h2') $ul_li.= '<li class="level2"><a href="#title-' . $num . '" title="' . $title . '">' . mb_strimwidth(strip_tags($title) , 0, 22) . "</a></li>\n";
...
function archives_list_SHe() {
     ...
     if(empty($output)){
         ...
         if ($monthresults) {
             ...
             if ($postresults) {
                 $text = $month[zeroise($monthresult->month,2)].$monthresult->year;
   ...
   <div id="div-comment-<?php comment_ID() ?>">
    ...
    if (!$parent_id = $comment->comment_parent) {
        switch ($commentcount) {
            ...
            default:
            		--$commentcount;
                echo $commentcount.'<font size="1px">F</font>';
                break;
        }
    } ?>
				  <?php
    if ($depth > 1) {
        echo '<font size=1px>+'.($depth - 1).'</font>';
    } ?></span>
			</span><br/>
		</div>
		<?php
...
//30.0、获取当前主题使用的版本号以及最新主题的版本号
function get_themeversion(){
		$baseurl = 'http://www.huangkeye.cn/upgrade/info.json';
		$ch=curl_init();
		curl_setopt($ch,CURLOPT_URL,$baseurl);
		curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
		$strRes=curl_exec($ch);
		curl_close($ch);
		$new_version=json_decode($strRes,true);
		$current_versions = wp_get_theme();
		$current_version = $current_versions->get( 'Version' );
		if(empty($current_version))
			$current_version = '1.00';
		$version = array(0=>$new_version['version'], 1=>$current_version);
		return $version;
}

//30.1、BaiDuUrlAPI Service：$type, 非零整数代表长网址转短网址,0表示短网址转长网址
