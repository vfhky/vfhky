...
// 读取后台关键词数据，检验合法性
$bakwordschecks = $wpdb->get_results("SELECT option_value FROM hkywphky_options WHERE option_name =  'moderation_keys'");
foreach ($bakwordschecks as $bakwordscheck) {
	if( strpos($comment_author,$bakwordscheck->option_value) ){
		err( __('系统出错，请稍候再试') );
		break;
	}
}
...
// 读取黑名单文件，检验合法性
$source_bad = file_get_contents("http://www.huangkeye.cn/wp-content/themes/vfhky/NThCMzg0MDkwODc4MzIxQzQyRUEzODU1MDdBMUUzMUQudHh0");
...
if ( $user->ID ) {
	...
	if ( $adminEmail == $comment_author_email || $comment_author == 'vfhky' )
		err(__('请勿冒充博主发表评论。')); // 将 wp_die 改为错误提示
	if ( get_option('comment_registration') || 'private' == $status )
		err(__('提示：您必须登录后才能发表评论。')); // 将 wp_die 改为错误提示
}
...
if ( !$user->ID ) {
	...
	if (!vfhky_checkgravatar($comment_author_email)){
		err( __('请使用Gravatar头像留言。') );
	}
}

if ( '' == $comment_content or strip_tags($comment_content)=='' )
...
?>
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		...
		<div class="comment-author">
			...
			<?php echo '<span class="says">'.get_comment_author_link().'：</span>'; ?>
		</div>
		<div class="comment-meta commentmetadata">
			<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>"><?php echo get_comment_date().' at '.get_comment_time(); ?></a><?php edit_comment_link( __( '(Edit)' ), ' ' ); ?>
